            ...
            {[0, 1, 2].map((i) => (
              <div
                ...
                className={`flex-1 h-1 rounded-full transition-all duration-300 ${
                  stepIndex >= i ? 'bg-gradient-to-r from-pink-500 to-purple-600' : 'bg-muted'
                }`}
              />
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-right transition-all ${
                          selected
                            ...
                            : 'border-border hover:border-pink-200 bg-card'
                        }`}
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all ${
                          selected ? 'bg-gradient-to-br from-pink-500 to-purple-600' : 'bg-muted'
                        }`}>
                        ...
                        <div className={`font-black text-lg shrink-0 ${selected ? 'text-pink-600' : 'text-muted-foreground'}`}>
                          {symbol}{s.price}
                        </div>
                      </button>
                    )
            ...
            {step === 'datetime' && (
              <motion.div key="datetime" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} className="pb-6">
                ...
                {selectedService && (
                  <div className="mx-6 mb-4 mt-2 flex items-center gap-2 bg-primary/10 border border-pink-100 rounded-xl px-3 py-2">
                ...
                <div className="px-6 mt-4">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full rounded-xl border border-border bg-muted px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-pink-300 focus:bg-background transition-colors resize-none"
                  />
                </div>
            ...
            {step === 'confirm' && selectedService && selectedDate && (
              <motion.div key="confirm" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} className="px-6 pb-6 pt-2">
                <div className="bg-primary/10 rounded-2xl p-5 space-y-3.5 mb-4 border border-pink-100">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-card border border-pink-100 flex items-center justify-center shrink-0">
                  ...
                  {notes && (
                    <>
                      ...
                      <div className="flex items-start gap-3">
                        <div className="w-8 h-8 rounded-xl bg-card border border-pink-100 flex items-center justify-center shrink-0 mt-0.5">
