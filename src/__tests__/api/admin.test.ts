...
function adminRequest(path = "/api/admin/stats") {
  ...
  Object.defineProperty(req, "cookies", {
    value: {
      get: (key: string) =>
        key === "auth-token" ? { value: "valid-token" } : undefined,
    },
  });
  return req;
}
...
function nonAdminRequest(path = "/api/admin/stats") {
  ...
  Object.defineProperty(req, "cookies", {
    value: {
      get: (key: string) =>
        key === "auth-token" ? { value: "non-admin-token" } : undefined,
    },
  });
  return req;
}
...
describe("GET /api/admin/users", () => {
  ...
  it("includes isAdmin: true for a user with the flag set, false otherwise", async () => {
    ...
    const res = await GET(adminRequest("/api/admin/users"));
    const json = await res.json();
    const alice = json.data.find(
      (u: { email: string }) => u.email === "alice@test.com",
    );
    const admin2 = json.data.find(
      (u: { email: string }) => u.email === "admin2@test.com",
    );
...
describe("GET /api/admin/nailists", () => {
  ...
  it("returns nailist list for admin", async () => {
    ...
    const json = await res.json();
    expect(Array.isArray(json.data)).toBe(true);
    expect(json.data[0].verificationReadiness).toMatchObject({
      totalCount: 9,
      isReady: false,
    });
  });
  ...
  it("returns factual readiness checks with exact missing criteria", async () => {
    ...
    collectionStore.services = [
      { __id: "s1", nailistProfileId: "n1", isActive: true },
    ];
    collectionStore.workingHours = [
      { __id: "h1", nailistProfileId: "n1", isActive: true },
    ];
    collectionStore.portfolioPhotos = Array.from({ length: 4 }, (_, index) => ({
      __id: `p${index}`,
      nailistProfileId: "n1",
    }));
    mockGetUsers.mockResolvedValue({
      users: [{ uid: "u1", emailVerified: true }],
    });
    ...
    expect(readiness.checks).toContainEqual(
      expect.objectContaining({
        key: "portfolio",
        passed: false,
        missing: "הוסיפי עוד 1 תמונה לתיק העבודות",
      }),
    );
  });
});
