...
// Content cascade: exit is a quick, un-staggered fade of the whole block
// (it's gone before the next mode's content mounts, since AnimatePresence's
// mode="wait" removes it from the DOM first) — entrance staggers in field by
// field via staggerChildren, which reads as considered rather than one flat
// block popping in at once.
const CASCADE_CONTAINER: Variants = {
  hidden: { transition: { duration: 0.18 } },
    ...
    <div className="min-h-screen flex bg-background">
      ...
      >
        <div className="w-full max-w-md">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* Heading */}
              <motion.div variants={CASCADE_ITEM} className="mb-6">
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-4">
                {mode === 'register' && (
                  <motion.div variants={CASCADE_ITEM} className="grid grid-cols-2 gap-3">
                ...
                <motion.div variants={CASCADE_ITEM} className="space-y-2">
                ...
                {mode === 'register' && (
                  <motion.label
                    ...
                    className={`flex items-start gap-2.5 cursor-pointer rounded-xl border px-3 py-2.5 transition-colors ${agreedToTerms ? 'border-primary/40 bg-primary/5' : 'border-border'}`}
                  >
                    <input
                      type="checkbox"
                ...
                <motion.div variants={CASCADE_ITEM}>
                  <Button
              ...
              <motion.div variants={CASCADE_ITEM} className="relative my-5">
              ...
              <motion.p variants={CASCADE_ITEM} className="text-center text-sm text-muted-foreground mt-5">
                {mode === 'login' ? (
                  <>אין לך חשבון עדיין?{' '}
      ...
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <motion.div variants={CASCADE_ITEM} className="mx-auto mb-7 w-16 h-16 rounded-2xl overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.2)]">
              <Image src="/logo.png" alt="נייליסטיות לוגו" width={64} height={64} className="w-full h-full object-cover" />
            </motion.div>
            <motion.h2 variants={CASCADE_ITEM} className="text-3xl font-black mb-2 leading-tight">{panel.heading}</motion.h2>
            <motion.p variants={CASCADE_ITEM} className="text-white/70 text-base mb-8">{panel.sub}</motion.p>
