...
import { Sparkles, ArrowLeft, Phone, MapPin, Loader2, Check } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { useAuth } from '@/components/auth/auth-provider'

const STEPS = [
  { label: 'פרטי יצירת קשר', emoji: '📱' },
  { label: 'מיקום', emoji: '📍' },
]
  ...
  async function handleFinish() {
    ...
    try {
      const res = await fetch('/api/me/client-profile', {
        ...
        body: JSON.stringify({ phoneNumber: phone, city: city || undefined }),
      })
            ...
            {done ? (
              ...
              >
                ...
                <div className="flex gap-3">
                  ...
                  >
                    {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : 'שמרי והמשיכי ✨'}
                  </Button>
                </div>
              </motion.div>
            )}
